...
def insert(filename):
    ...
    tdict['Cost:'] = cost
    with open(filename, 'ab') as fobj: pickle.dump(tdict, fobj)
...
def conditional_del(filename):
    ...
    try:
        while True:
            dat = pickle.load(fobj)
            if dat['Source:'] == 'Delhi' and dat['Destination:'] == 'Mumbai':
                found = True  # Conditional deletion.
                print("Deleting...")
                pass
            else:
             pickle.dump(dat, tfobj)
...
def cost_inflate(filename):
    ...
    try:
        while True:
            ...
            dat = pickle.load(fobj)
            dat['Cost:'] += dat['Cost:'] * (2 / 10)  # Inflation. fobj.seek(rpos)
            pickle.dump(dat, fobj)
    except FileNotFoundError:
        print("File not found.")
    except EOFError:
        print("End of file.")
...
while True:
    ...
    elif uc == '7':  # New options.
        print("Conditional deletion...")
        filename = input("Enter filename with .dat extension: ")
        conditional_del(filename)
    elif uc == '8':
        filename = input("Enter filename with .dat extension: ")
        print("No. of records: ", recno(filename))
    elif uc == '9':
        filename = input("Enter filename with .dat extension: ")
        cost_inflate(filename)
    elif uc == '10':
        print("Exiting...")
        break
    else:
